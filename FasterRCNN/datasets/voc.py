...
class Dataset:
  ...
  def __init__(self, split, image_preprocessing_params, compute_feature_map_shape_fn, feature_pixels = 16, dir = "dataset", augment = False, shuffle = True, allow_difficult = False, cache = True):    
    """
    Parameters
    ----------
    split : str
  ...
  def _generate_training_sample(self, filepath, flip):
    ...
    scaled_image_data, scaled_image, scale_factor, original_shape = image.load_image(url = filepath, preprocessing = self._image_preprocessing_params, min_dimension_pixels = None, horizontal_flip = flip)
  ...
  def _get_classes(self):
    imageset_dir = os.path.join(self._dir, "datasplit")
    classes = set([ os.path.basename(path).split("_")[0] for path in Path(imageset_dir).glob("*_" + self.split + ".txt") ])
    assert len(classes) > 0, "No classes found in datasplit for '%s' split" % self.split
  ...
  def _get_filepaths(self):
    image_list_file = os.path.join(self._dir, "datasplit", self.split + ".txt")
    with open(image_list_file) as fp:
      basenames = [ line.strip() for line in fp.readlines() ] # strip newlines
    image_paths = [ os.path.join(self._dir, "images", basename) + ".jpg" for basename in basenames ]
    return image_paths
